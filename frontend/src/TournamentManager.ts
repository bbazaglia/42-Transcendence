...
export class TournamentManager {
  ...
  private loadTournament(): void {
    ...
    if (saved) {
      try {
        ...
        console.log('Tournament loaded from localStorage:', this.currentTournament)
      } catch (e) {
  ...
  recordMatchResult(player1: string, player2: string, winner:string): void {
    ...
    if (match) {
      ...
      //     console.log(`Match ${i}: ${m.player1} vs ${m.player2}`);
      //   }
      // });
    }
  }
  ...
  private advanceWinner(winner: string, currentMatch: Match): void {
    ...
    const finalMatch = this.currentTournament.matches[this.currentTournament.matches.length - 1]
