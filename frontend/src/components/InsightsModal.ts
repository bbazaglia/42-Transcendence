...
export class InsightsModal {
    ...
    private createModal(): void {
        ...
        this.modal.className = 'fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4';
    ...
    private async loadAnalytics(): Promise<void> {
        const loadingEl = this.modal.querySelector('#insights-loading') as HTMLElement;
        const contentEl = this.modal.querySelector('#insights-content') as HTMLElement;
    ...
    private renderAnalytics(): void {
        ...
        const { overview, performanceOverTime, scoreDistribution, opponentAnalysis, recentTrends } = this.analytics;
    ...
    private updateOverviewCards(overview: any): void {
        ...
        if (totalMatchesEl) totalMatchesEl.textContent = overview.totalMatches.toString();
        if (winRateEl) winRateEl.textContent = analyticsService.formatWinRate(overview.winRate);
    ...
    private createPerformanceChart(performanceOverTime: any[]): void {
        ...
        const chart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: performanceOverTime.map(p => new Date(p.date).toLocaleDateString()),
                datasets: [{
                    label: 'Win Rate %',
    ...
    private createWinLossChart(overview: any): void {
        const canvas = this.modal.querySelector('#winloss-chart') as HTMLCanvasElement;
        if (!canvas) return;
    ...
    private createScoreChart(scoreDistribution: any): void {
        const canvas = this.modal.querySelector('#score-chart') as HTMLCanvasElement;
        if (!canvas) return;
        ...
        const allScores = [...scoreDistribution.playerOneScores, ...scoreDistribution.playerTwoScores];
    ...
    private createScoreRanges(scores: number[]): Array<{min: number, max: number, count: number}> {
        ...
        for (let i = min; i <= max; i += rangeSize) {
            ...
            const count = scores.filter(score => score >= i && score <= rangeMax).length;
    ...
    private renderOpponentAnalysis(opponentAnalysis: any[]): void {
        ...
        container.innerHTML = topOpponents.map(opponent => `
            <div class="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/10">
                <div class="flex items-center space-x-4">
                    <div class="w-10 h-10 bg-gradient-to-br from-cyan-400 to-purple-600 rounded-full flex items-center justify-center">
                        <span class="text-white font-bold text-sm">${opponent.opponentName.charAt(0).toUpperCase()}</span>
                    </div>
                    <div>
                        <div class="text-white font-medium">${opponent.opponentName}</div>
                        <div class="text-gray-400 text-sm">${opponent.matchesPlayed} matches</div>
                    </div>
                </div>
                <div class="text-right">
                    <div class="text-cyan-400 font-bold">${analyticsService.formatWinRate(opponent.winRate)}</div>
                    <div class="text-gray-400 text-sm">${opponent.wins}W - ${opponent.losses}L</div>
                </div>
            </div>
        `).join('');
    }
            ...
            trend7El.innerHTML = `
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-cyan-400">${analyticsService.formatWinRate(trend7.winRate)}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-400">Record:</span>
                        <span class="text-white">${trend7.wins}W - ${trend7.losses}L</span>
                    </div>
                </div>
            `;
        }
        ...
        if (trend30El) {
            ...
            trend30El.innerHTML = `
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between">
                        <span class="text-gray-400">Win Rate:</span>
                        <span class="text-cyan-400">${analyticsService.formatWinRate(trend30.winRate)}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-400">Record:</span>
                        <span class="text-white">${trend30.wins}W - ${trend30.losses}L</span>
                    </div>
                </div>
            `;
        }
